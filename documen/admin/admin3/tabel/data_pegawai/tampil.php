...
<body>
	...
	<a target="blank" href="cetak.php?berdasarkan=data_pegawai&jenis=xls&pakaiperperiode=<?php echo $status_pakaiperperiode;?>">
	<?php btn_export('Export Excel'); ?>
	</a>
	
	<a target="blank" href="cetak.php?berdasarkan=data_pegawai&jenis=print&pakaiperperiode=<?php echo $status_pakaiperperiode;?>">
	<?php btn_cetak('Cetak'); ?>
	</a>
...
<th align="center" class="th_border cell"  >Password</th>
			...
			<?php
				...
				$querytabel="SELECT * FROM data_pegawai where $berdasarkan like '%$isi%'  LIMIT $startRow ,$dataPerPage";
				$querypagination="SELECT COUNT(*) AS total FROM data_pegawai where $berdasarkan like '%$isi%'";
				}
				else
				{
				$querytabel="SELECT * FROM data_pegawai  LIMIT $startRow ,$dataPerPage";
				$querypagination="SELECT COUNT(*) AS total FROM data_pegawai";
				}
               ...
               <tr class="event2">	
				  <td class="th_border cell" align="center" width="200">
						<table border="0">
							<tr>
								<td>
									<a href="<?php index(); ?>?input=detail&proses=<?= encrypt($data['id_pegawai']); ?>">
                 ...
                 <td align="center"><?php echo (substr($data['NIP'],0,100)); ?></td>
<td align="center"><?php echo (substr($data['nama'],0,100)); ?></td>
<td align="center"><?php echo (substr($data['alamat'],0,100)); ?></td>
<td align="center"><?php echo (substr($data['tempat_lahir'],0,100)); ?></td>
<td align="center"><?php echo (format_indo($data['tanggal_lahir'])); ?></td>
<td align="center"><?php echo (substr($data['jenis_kelamin'],0,100)); ?></td>
<td align="center"><?php echo (substr($data['jabatan'],0,100)); ?></td>
<td align="center"><?php echo (substr($data['username'],0,100)); ?></td>
