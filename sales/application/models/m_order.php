...
class M_order extends CI_Model{
	...
	function get_myhistory($kopel){
		$hsl=$this->db->query("SELECT * FROM tbl_invoice JOIN tbl_status ON status_id = inv_status WHERE inv_plg_id='$kopel' ORDER BY DATE(inv_tanggal) DESC");
		return $hsl;
	}

	function invoice_list($invoice){
		$hasil=$this->db->query("SELECT * FROM tbl_detail WHERE detail_inv_no='$invoice'");
		return $hasil->result();
	}

	function get_total_invoice($invoice){
		$hasil=$this->db->query("SELECT * FROM tbl_invoice WHERE inv_no='$invoice'");
		return $hasil->result();
	}
	...
	function get_total_jumlah_terjual_bulan_ini(){
		$hsl=$this->db->query("SELECT SUM(detail_jumlah) AS total_jumlah FROM tbl_invoice JOIN tbl_detail ON inv_no=detail_inv_no WHERE MONTH(inv_tanggal)=MONTH(CURDATE())");
		return $hsl;
	}
	...
	function get_all_order(){
		$hsl=$this->db->query("SELECT inv_no,DATE_FORMAT(inv_tanggal,'%d %M %Y') AS inv_tanggal,inv_plg_id,inv_plg_nama,inv_status,inv_total FROM tbl_invoice ORDER BY DATE(inv_tanggal) DESC LIMIT 6");
		return $hsl;
	}

	function get_order(){
		$hsl=$this->db->query("SELECT inv_no,DATE_FORMAT(inv_tanggal,'%d %M %Y') AS inv_tanggal,inv_plg_id,inv_plg_nama,inv_status,inv_total,status_nama FROM tbl_invoice JOIN tbl_status ON status_id = inv_status ORDER BY DATE(inv_tanggal) DESC");
		return $hsl;
	}
	...
	function order_produk($no_invoice,$total){
		...
		$this->db->query("INSERT INTO tbl_invoice (inv_no,inv_plg_id,inv_plg_nama,inv_status,inv_total) VALUES ('$no_invoice','$kopel','$napel','1','$total')");
		foreach ($this->cart->contents() as $item) {
			$data=array(
				'detail_produk_id'	 =>	$item['id'],
				'detail_produk_nama'   =>	$item['name'],
	...
	function get_checkout($no_invoice){
		$hsl=$this->db->query("SELECT inv_no,DATE_FORMAT(inv_tanggal,'%d/%m/%Y') AS tanggal,inv_plg_nama,inv_status,inv_total,inv_rek_id,inv_rek_no,inv_rek_bank,inv_rek_nama,inv_rek_cabang,detail_produk_id,detail_produk_nama,detail_harjul,detail_jumlah,detail_subtotal FROM tbl_invoice JOIN tbl_detail ON inv_no=detail_inv_no WHERE inv_no='$no_invoice'");
		return $hsl;
	}
	...
	function hapus_sup($kode){
		$hsl=$this->db->query("delete from tbl_trans_bb where id='$kode'");
		return $hsl;
	}
